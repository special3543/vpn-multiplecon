...
function chooseSubnet() {
    echo "Please choose the subnet you want to use for OpenVPN:"
    echo "   1) 10.8.0.0/22 - 1022 hosts"
    echo "   2) 10.8.0.0/21 - 2046 hosts"
    echo "   3) 10.8.0.0/20 - 4094 hosts"
    echo "   4) 10.8.0.0/19 - 8190 hosts"
    echo "   5) 10.8.0.0/18 - 16382 hosts"
    echo "   6) 10.8.0.0/17 - 32766 hosts"
    echo "   7) 10.8.0.0/16 - 65534 hosts"

    read -rp "Subnet choice [1-7]: " -e SUBNET_CHOICE

    case $SUBNET_CHOICE in
    1)
        VPN_SUBNET="10.8.0.0 255.255.252.0"
        UNBOUND_ACCESS="10.8.0.1/22"
		IPTABLES_SUBNET="10.8.0.0/22"
        ;;
    2)
        VPN_SUBNET="10.8.0.0 255.255.248.0"
        UNBOUND_ACCESS="10.8.0.1/21"
		IPTABLES_SUBNET="10.8.0.0/21"
        ;;
    3)
        VPN_SUBNET="10.8.0.0 255.255.240.0"
        UNBOUND_ACCESS="10.8.0.1/20"
		IPTABLES_SUBNET="10.8.0.0/20"
        ;;
    4)
        VPN_SUBNET="10.8.0.0 255.255.224.0"
        UNBOUND_ACCESS="10.8.0.1/19"
		IPTABLES_SUBNET="10.8.0.0/19"
        ;;
    5)
        VPN_SUBNET="10.8.0.0 255.255.192.0"
        UNBOUND_ACCESS="10.8.0.1/18"
		IPTABLES_SUBNET="10.8.0.0/18"
        ;;
    6)
        VPN_SUBNET="10.8.0.0 255.255.128.0"
        UNBOUND_ACCESS="10.8.0.1/17"
		IPTABLES_SUBNET="10.8.0.0/17"
        ;;
    7)
        VPN_SUBNET="10.8.0.0 255.255.0.0"
        UNBOUND_ACCESS="10.8.0.1/16"
		IPTABLES_SUBNET="10.8.0.0/16"
        ;;
    *)
        echo "Invalid choice, defaulting to 10.8.0.0/22"
        VPN_SUBNET="10.8.0.0 255.255.252.0"
        UNBOUND_ACCESS="10.8.0.1/22"
		IPTABLES_SUBNET="10.8.0.0/22"
        ;;
...
access-control: '"$UNBOUND_ACCESS"' allow
hide-identity: yes
hide-version: yes
use-caps-for-id: yes
prefetch: yes' >>/etc/unbound/unbound.conf
			...
			sed -i 's|# access-control: 127.0.0.0/8 allow|access-control: '"$UNBOUND_ACCESS"' allow|' /etc/unbound/unbound.conf
...
duplicate-cn" >>/etc/openvpn/server.conf
	...
	# Script to add rules
	echo "#!/bin/sh
iptables -t nat -I POSTROUTING 1 -s $IPTABLES_SUBNET -o $NIC -j MASQUERADE
...
ip6tables -I INPUT 1 -i $NIC -p $PROTOCOL --dport $PORT -j ACCEPT" >>/etc/iptables/add-openvpn-rules.sh
	...
	echo "#!/bin/sh
iptables -t nat -D POSTROUTING -s $IPTABLES_SUBNET -o $NIC -j MASQUERADE
